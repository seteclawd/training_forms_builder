    ...
    case 'db_acReg': case 'db_adIcao': case 'db_acType':
      {
        ...
        html += `        </select>\n`;
        html += `<script>setTimeout(function(){if('${dbName}'==='location'||'${dbName}'==='fstdId'){fetch('/api/crew?source=${dbName}').then(r=>r.json()).then(rows=>{var s=document.getElementById('${selId}');if(!s)return;s.innerHTML='<option value="">-- Select --</option>';rows.forEach(r=>{s.innerHTML+='<option value="'+r.name+'">'+r.name+'</option>';});});}else{fetch('/api/crew?source=${dbName}').then(r=>r.json()).then(rows=>{var s=document.getElementById('${selId}');if(!s)return;s.innerHTML='<option value="">-- Select --</option>';rows.forEach(r=>{var v='${dbName}'==='crewName'?r.name:'${dbName}'==='crew3lc'||'${dbName}'==='crewId'?r.three_lc:'${dbName}'==='crewLicense'?(r.license_number||''):'${dbName}'==='acReg'?(r.ac_reg||''):'${dbName}'==='adIcao'?(r.ad_icao||''):'${dbName}'==='acType'?(r.ac_type||''):r.name;var l='${dbName}'==='crew3lc'||'${dbName}'==='crewId'?r.three_lc+' - '+r.name:'${dbName}'==='instructorTri'?(r.name==='GFO'||r.is_sfi?'SFI':'TRI')+' - '+r.name:'${dbName}'==='examinerTre'?(r.name==='GFO'||r.is_sfe?'SFE':'TRE')+' - '+r.name:'${dbName}'==='acReg'?r.ac_reg:'${dbName}'==='adIcao'?r.ad_icao:'${dbName}'==='acType'?r.ac_type:r.name+(r.position?' ('+r.position+')':'');if('${dbName}'==='acType'||'${dbName}'==='acReg'||'${dbName}'==='adIcao'){if(!window['_seen_'+id])window['_seen_'+id]={};if(v&&!window['_seen_'+id][v]){window['_seen_'+id][v]=true;s.innerHTML+='<option value="'+v+'">'+l+'</option>';}}else{s.innerHTML+='<option value="'+v+'">'+l+'</option>';}if('${dbName}'==='acType'||'${dbName}'==='acReg'||'${dbName}'==='adIcao'){if(!window._seenOpt_${selId})window._seenOpt_${selId}={};if(!window._seenOpt_${selId}[v]){window._seenOpt_${selId}[v]=1;s.innerHTML+='<option value="'+v+'">'+l+'</option>';}}else{s.innerHTML+='<option value="'+v+'">'+l+'</option>';}if('${dbName}'==='crewName'){if(!window._crewData)window._crewData=rows;s.addEventListener('change',function(){var sel=rows.find(x=>x.name===s.value);var form=s.closest('form')||s.closest('.builder-canvas')||s.closest('.modal-content')||document.body;var posSel=form.querySelector('select[data-field-type="pilotPosition"]');var licSel=form.querySelector('select[data-field-type="crewLicense"]');var tlcSel=form.querySelector('select[data-field-type="crew3lc"]');setTimeout(function(){if(posSel&&sel)posSel.value=sel.position||'';if(licSel&&sel)licSel.value=sel.license_number||'';if(tlcSel&&sel)tlcSel.value=sel.three_lc||'';},300);});}if('${dbName}'==='acType'){if(!window._allCrewData)window._allCrewData=rows;s.addEventListener('change',function(){var selectedType=s.value;var form=s.closest('form')||s.closest('.builder-canvas')||s.closest('.modal-content')||document.body;var regSel=form.querySelector('select[data-field-type="acReg"]');if(regSel){regSel.innerHTML='<option value="">-- Select --</option>';var filtered=selectedType?window._allCrewData.filter(function(r){return r.ac_type===selectedType;}):window._allCrewData;var seen={};filtered.forEach(function(r){if(r.ac_reg&&!seen[r.ac_reg]){seen[r.ac_reg]=true;regSel.innerHTML+='<option value="'+r.ac_reg+'">'+r.ac_reg+'</option>';}});}});}}).catch(e=>{var s=document.getElementById('${selId}');if(s)s.innerHTML='<option value="">-- Error --</option>';});}},100);</script>\n`;
      }
      break;

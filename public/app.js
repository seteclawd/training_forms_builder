...
function designTableField(fieldId) {
    ...
    <div style="background:#1e293b;border-radius:12px;width:98vw;max-width:1400px;height:90vh;display:flex;flex-direction:column;border:1px solid #334155;">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 20px;border-top:1px solid #334155;">
        <div style="display:flex;gap:8px;align-items:center;">
          <span style="color:#93c5fd;font-size:0.8rem;">Assign field to selected cell:</span>
          <select id="fdAssignSelect" style="padding:4px 8px;border:1px solid #475569;border-radius:4px;background:#0f172a;color:#e2e8f0;font-size:0.8rem;">
            ...
            <optgroup label="Database Fields">
              <option value="db_crewName">👤 Crew Name</option>
              <option value="db_crew3lc">👤 Crew 3LC</option>
              <option value="db_crewLicense">🪪 License</option>
              <option value="db_pilotPosition">💺 Position</option>
              <option value="db_acType">✈️ A/C Type</option>
              <option value="db_acReg">✈️ A/C Reg</option>
              <option value="db_instructorTri">🎓 Instructor (TRI)</option>
              <option value="db_examinerTre">📝 Examiner (TRE)</option>
              <option value="db_adIcao">🌍 AD/ICAO</option>
            </optgroup>
            <optgroup label="Trainer">
              <option value="trainer">🎓 TRI / TRE / SFI</option>
            </optgroup>
          </select>
          <button id="fdAssignBtn" style="padding:4px 12px;background:#1d4ed8;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:0.8rem;">Assign</button>
        </div>
  ...
  tinymce.init({
    ...
    init_instance_callback: function(editor) {
      // Field assignment button
      document.getElementById('fdAssignBtn').addEventListener('click', () => {
        const select = document.getElementById('fdAssignSelect');
        const type = select.value;
        if (!type) { alert('Select a field type first'); return; }
        const td = window._fdSelectedCell;
        if (!td) { alert('Select a cell in the table first'); return; }
        ...
        td.innerHTML = '<span class="cell-field-badge" style="color:#3b82f6;font-weight:bold;font-size:0.85rem;">' + label + '</span>';
